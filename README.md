# Cybersecurity Portfolio

## Introduction
Welcome to my cybersecurity portfolio. As a Carnegie Mellon University graduate with a B.S. in Electrical and Computer Engineering, I bring a rigorous engineering foundation to the world of cybersecurity. I am dedicated to identifying vulnerabilities and securing complex systems through a deep understanding of both cyber defense and offensive methodologies.

## Skills
- **Vulnerability Assessment**
- **Vulnerability Management**
- **Tools of the Trade**: Wazuh, tcpdump, VirusTotal, Suricata, nmap
- **Scripting & Automation**: C, Python, SQL
- **Incident Response**
- **Core Concepts and Frameworks** NIST CSF, CIS Controls, IAM, Zero-Trust

## Portfolio Sections

### 📜   Certificate Related Work
Documentation and projects associated with various cybersecurity certifications.
[View Certifications](./Certificates/Certifications.md)
- [GRC Portfolio:](./GRC/) Projects related to the governance, risk, and compliance domain.
- [IAM Portfolio:](./IAM_SysAdm_SecOps/) Projects related to identity and access management and system administration.
- [SOC Portfolio:](./SOC/) Projects related to cybersecurity operations and incident response.
- [Vulnerability Management Portfolio:](./VulnMgmt/) Projects related to vulnerability management and reducing organizations' attack surface.

### 🚀 Personal Projects
A collection of personal projects, tools, and write-ups showcasing my practical skills and learning journey.
- [Phishing Bot Utility for Cybersecurity Training](https://github.com/DanSaad/angler-education/blob/main/README.md)

---
[LinkedIn](linkedin.com/in/dan-saad) | dsaad@alumni.cmu.edu